...
- [x] **进程状态指示灯优化**
  ...
  - 强制终止：橙色
- [x] **进程日志过滤功能**
  - 添加过滤器：按级别过滤（INFO/SUCCESS/WARNING/ERROR/DEBUG/STDOUT/STDERR）
  - 添加搜索框：关键字高亮（黄色背景）
- [x] **进程日志导出功能**
  - 导出为 .txt 文件（UTF-8 编码）
  - 包含时间戳、日志级别和进程标识
...
#### 1.2 配置文件语法高亮
- [x] 源码模式下添加语法高亮
  - INI 格式高亮（section/key/value/comment）
  - YAML 格式高亮（key/value/list/comment）
  - JSON 格式高亮（key/string/number/boolean/null）
- [x] 行号显示（CodeEditor 组件）
- [ ] 错误行标记（验证失败时高亮错误行）
